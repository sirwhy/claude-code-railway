...
class Settings(BaseSettings):
    ...
    anthropic_base_url: Optional[str] = Field(
        None,
        description=(
            "Custom Anthropic API base URL. Set this to use a proxy or custom endpoint "
            "(e.g. https://cli-proxy-api-production-d321.up.railway.app/v1). "
            "Injects ANTHROPIC_BASE_URL into the environment for the SDK."
        ),
    )
    ...
    def validate_approved_directory(cls, v: Any) -> Path:
        """Ensure approved directory exists and is absolute. Auto-creates if missing."""
        if isinstance(v, str):
            v = Path(v)

        path = v.resolve()
        if not path.exists():
            # On Railway (or any container env) the directory may not exist yet —
            # create it automatically instead of raising an error.
            try:
                path.mkdir(parents=True, exist_ok=True)
            except Exception:
                raise ValueError(f"Approved directory does not exist and could not be created: {path}")
        if not path.is_dir():
    ...
    def anthropic_base_url_str(self) -> Optional[str]:
        """Get Anthropic base URL as string (stripped of trailing slash)."""
