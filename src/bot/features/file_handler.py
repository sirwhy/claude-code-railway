...
class FileHandler:
    ...
    async def handle_document_upload(
        ...
        """Process uploaded document — accepts ALL file types."""
        ...
        try:
            ...
            if file_type == "archive":
                ...
                # Binary file (pdf, docx, xlsx, images, executables, etc.)
                # Copy to workspace so Claude can access it via Read/Bash tools
                return await self._process_binary_file(file_path, context)

        finally:
            # Cleanup temp copy (workspace copy is kept)
    ...
    async def _process_binary_file(self, file_path: Path, context: str) -> ProcessedFile:
        """Process any binary file by copying it to the workspace directory.

        Claude can then use Read, Bash, or other tools to work with the file
        (e.g. read a PDF with pdfplumber, parse Excel with openpyxl, etc).
        """
        import shutil as _shutil

        # Copy file into the approved workspace so Claude tools can access it
        workspace_path = self.config.approved_directory / file_path.name
        _shutil.copy2(file_path, workspace_path)

        ext = file_path.suffix.lower()
        size = file_path.stat().st_size

        # Build a helpful prompt that tells Claude exactly where the file is
        hint = ""
        if ext == ".pdf":
            hint = "Gunakan Bash tool dengan `python3 -c \"import pdfplumber; ...\"`  atau `pdftotext` untuk membacanya."
        elif ext in (".xlsx", ".xls"):
            hint = "Gunakan Bash tool dengan `python3 -c \"import openpyxl; wb=openpyxl.load_workbook(...)\"` untuk membacanya."
        elif ext in (".docx", ".doc"):
            hint = "Gunakan Bash tool dengan `python3 -c \"from docx import Document; doc=Document(...)\"` untuk membacanya."
        elif ext in (".jpg", ".jpeg", ".png", ".gif", ".webp", ".bmp"):
            hint = "Ini adalah file gambar. Gunakan tool yang sesuai untuk menganalisisnya."
        else:
            hint = f"File binary dengan ekstensi {ext}. Gunakan Bash tool untuk memproses sesuai kebutuhan."

        prompt = (
            f"{context}\n\n"
            f"File telah diupload ke workspace: `{workspace_path}`\n"
            f"Nama file: {file_path.name}\n"
            f"Ukuran: {self._format_size(size)}\n"
            f"Tipe: {ext or 'unknown'}\n\n"
            f"{hint}"
        )
