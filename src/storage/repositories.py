...
PostgreSQL/SQLite compatible - query placeholders and functions are handled
by the DatabaseManager layer. Only the cleanup_old_sessions query needed
a small PG-specific tweak (interval syntax), everything else is standard SQL.
"""
...
class SessionRepository:
    ...
    async def create_session(self, session: SessionModel) -> SessionModel:
        async with self.db.get_connection() as conn:
            ...
            await conn.commit()
            logger.info("Created session", session_id=session.session_id, user_id=session.user_id)
            return session
    ...
    async def cleanup_old_sessions(self, days: int = 30) -> int:
        """Mark old sessions as inactive. Uses portable SQL handled by DB layer."""
        async with self.db.get_connection() as conn:
            # DatabaseManager._sqlite_to_pg converts this automatically for PG
            cursor = await conn.execute(
                """
                UPDATE sessions
                SET is_active = FALSE
...
class ProjectThreadRepository:
    ...
    async def get_by_chat_thread(self, chat_id: int, message_thread_id: int) -> Optional[ProjectThreadModel]:
        async with self.db.get_connection() as conn:
            cursor = await conn.execute(
                """
    ...
    async def get_by_chat_project(self, chat_id: int, project_slug: str) -> Optional[ProjectThreadModel]:
        async with self.db.get_connection() as conn:
            cursor = await conn.execute(
                "SELECT * FROM project_threads WHERE chat_id = ? AND project_slug = ?",
    ...
    async def list_stale_active_mappings(self, chat_id: int, active_project_slugs: List[str]) -> List[ProjectThreadModel]:
        async with self.db.get_connection() as conn:
            if active_project_slugs:
                placeholders = ",".join("?" for _ in active_project_slugs)
                query = f"""
                ...
                cursor = await conn.execute(
                    "SELECT * FROM project_threads WHERE chat_id = ? AND is_active = TRUE ORDER BY project_slug ASC",
                    (chat_id,),
                )
            rows = await cursor.fetchall()
...
class ToolUsageRepository:
    ...
    async def get_session_tool_usage(self, session_id: str) -> List[ToolUsageModel]:
        async with self.db.get_connection() as conn:
            cursor = await conn.execute(
                "SELECT * FROM tool_usage WHERE session_id = ? ORDER BY timestamp DESC",
                (session_id,),
            )
            rows = await cursor.fetchall()
...
class CostTrackingRepository:
    ...
    async def update_daily_cost(self, user_id: int, cost: float, date: str = None):
        ...
        async with self.db.get_connection() as conn:
            await conn.execute(
                ...
                DO UPDATE SET
                    daily_cost = cost_tracking.daily_cost + ?,
    ...
    async def get_total_costs(self, days: int = 30) -> List[Dict]:
        async with self.db.get_connection() as conn:
            cursor = await conn.execute(
                """
                SELECT
...
class AnalyticsRepository:
    ...
    async def get_user_stats(self, user_id: int) -> Dict:
        async with self.db.get_connection() as conn:
            ...
            cursor = await conn.execute(
                """
                SELECT tu.tool_name, COUNT(*) as usage_count
                FROM tool_usage tu
                JOIN sessions s ON tu.session_id = s.session_id
                WHERE s.user_id = ?
            ...
            return {"summary": summary, "daily_usage": daily_usage, "top_tools": top_tools}
    ...
    async def get_system_stats(self) -> Dict:
        async with self.db.get_connection() as conn:
            ...
            cursor = await conn.execute(
                "SELECT COUNT(DISTINCT user_id) as active_users FROM messages WHERE timestamp > datetime('now', '-7 days')"
            )
            row = await cursor.fetchone()
            overall["active_users_7d"] = row[0] if row else 0

            cursor = await conn.execute(
                """
                SELECT u.user_id, u.telegram_username,
                       SUM(m.cost) as total_cost, COUNT(m.message_id) as total_messages
                FROM messages m
            ...
            cursor = await conn.execute(
                """
                SELECT tool_name, COUNT(*) as usage_count, COUNT(DISTINCT session_id) as sessions_used
                FROM tool_usage
                GROUP BY tool_name
                ORDER BY usage_count DESC
                LIMIT 10
